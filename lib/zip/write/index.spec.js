...
import readStat from '../read-stat/index.js';
...
test('redzip: zip: write: directory: mtime', async (t) => {
    const originalOuterPath = new URL('fixture/hello.zip', import.meta.url).pathname;
    const outerPath = '/fixture/hello.zip';
    const innerPath = '/world/';
    const restoreFs = await mockFile(originalOuterPath);
    ...
    const {date} = await readStat(outerPath, innerPath);
...
test('redzip: zip: write: date', async (t) => {
    const originalOuterPath = new URL('fixture/hello.zip', import.meta.url).pathname;
    const outerPath = '/fixture/hello.zip';
    const innerPath = '/world';
    const restoreFs = await mockFile(originalOuterPath);
    ...
    const {date} = await readStat(outerPath, '/hello.txt');
