...
test('redzip: local: remove: error', async (t) => {
    ...
    t.equal(error.message, `EACCES: permission denied, unlink '/root/xxx'`);
    t.end();
});
...
test('redzip: local: remove: file: EPERM', async (t) => {
    const innerPath = '';
    const outerPath = join(__dirname, 'fixture', 'remove-me');
    
    const mockedError = Error('');
    mockedError.code = 'EPERM';
    
    const unlink = stub().throws(mockedError);
