...
test('redzip: local: write: date', async (t) => {
    const name = join(tmpdir(), 'redzip-remove-me.txt');
    const data = 'hello';
    const innerPath = '';
...
test('redzip: local: write: date: nested', async (t) => {
    const name = join(tmpdir(), 'hello', 'world', 'redzip-remove-me.txt');
    const data = 'hello';
    const innerPath = '';
...
test('redzip: local: write: file: nested: date', async (t) => {
    const dirPath = join(tmpdir(), 'hello');
    const name = join(dirPath, 'redzip-remove-me.txt');
    const data = 'hello';
    const innerPath = '';
    
    const mtime = new Date(1996, 1, 29);
    
    await write(dirPath, innerPath, null, {
        date: mtime,
    });
    
    await write(name, innerPath, Readable.from(data));
    
    const {date} = await readStat(dirPath);
...
test('redzip: local: write: dir: nested: date', async (t) => {
    const dirPath = join(tmpdir(), 'hello');
    const name = join(dirPath, 'world');
    
    const innerPath = '';
    
    const mtime = new Date(1996, 1, 29);
    
    await write(dirPath, dirPath, null, {
        date: mtime,
    });
...
test('redzip: local: write: directory: date', async (t) => {
    const name = join(tmpdir(), 'redzip-remove-me');
    const innerPath = '';
